function [rec,prec,ap] = x10VOCevaldet(VOCopts,id,cls,draw)
  ...
  if exist(cp,'file')
    ...
    path = sprintf(VOCopts.imgsetpath,VOCopts.testset) ;
    [gtids,t]=textread(path ,'%s %d') ; %#ok
    for i=1:length(gtids)
      if toc > 1 % display progress
  ...
  hash = xVOChash_init(gtids) ; % hash image ids
  ...
  % load results
  path = sprintf(VOCopts.detrespath,id,cls) ;
  [ids,confidence,b1,b2,b3,b4] = textread(path,'%s %f %f %f %f %f') ;
  BB = [b1 b2 b3 b4]' ;

  % sort detections by decreasing confidence
  [~,si] = sort(-confidence) ; ids = ids(si) ; BB = BB(:,si) ;

  % assign detections to ground truth objects
  nd=length(confidence) ; tp = zeros(nd,1) ; fp = zeros(nd,1) ; tic ;
  ...
  for d=1:nd
    ...
    i = xVOChash_lookup(hash,ids{d}) ; % find ground truth image
    if isempty(i), error('unrecognized image "%s"',ids{d}) ;
    elseif length(i)>1, error('multiple image "%s"',ids{d}) ;
    end
    ...
    for j=1:size(gt(i).BB,2)
      ...
      if iw>0 && ih>0                
        ...
        ua=(bb(3)-bb(1)+1)*(bb(4)-bb(2)+1)+...
           ...
           iw*ih ;
        ov = iw*ih/ua ;
        if ov>ovmax, ovmax = ov ; jmax = j ; end
      end
    end
  ...
  ap = xVOCap(rec,prec) ;
  if draw % plot precision/recall
    plot(rec,prec,'-') ; grid ; xlabel('recall') ; ylabel('precision') ;
